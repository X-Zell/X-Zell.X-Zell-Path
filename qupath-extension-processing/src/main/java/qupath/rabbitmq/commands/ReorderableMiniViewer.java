...
public class ReorderableMiniViewer {

    private static final Logger logger = LoggerFactory.getLogger(ReorderableMiniViewer.class);

    private static BooleanProperty showAllChannels = PathPrefs.createPersistentPreference("channelViewerAllChannels", false);

    private static String[] orderedChannelNames;
    ...
    static Stage createDialog(QuPathViewer viewer, String[] orderedChannelNames_) {

        orderedChannelNames = orderedChannelNames_;
        ...
        ReorderableMiniViewer.MiniViewerManager manager = createManager(viewer, channels);
    ...
    private static void updateChannels(QuPathViewer viewer, ReorderableMiniViewer.MiniViewerManager manager, Scene scene) {
    ...
    private static List<ChannelDisplayInfo> getChannels(ImageDisplay display) {
        List<ChannelDisplayInfo> channels = getChannels(display, showAllChannels.get());
        if (orderedChannelNames.length > 0) {
            List<String> orderedChannelNamesList = List.of(orderedChannelNames);
            channels = reorderChannels(channels, orderedChannelNamesList);
        }
        return channels;
    }

    public static List<ChannelDisplayInfo> reorderChannels(List<ChannelDisplayInfo> displayInfoList, List<String> desiredOrder) {
        Map<String, ChannelDisplayInfo> displayInfoMap = new HashMap<>();

        // Create a map for quick access using channel names
        for (ChannelDisplayInfo info : displayInfoList) {
            displayInfoMap.put(info.getName(), info);
        }

        List<ChannelDisplayInfo> reorderedList = new ArrayList<>();

        // Reorder based on the desiredOrder
        for (String name : desiredOrder) {
            ChannelDisplayInfo info = getChannelByPrefix(name, displayInfoMap);
            if (info != null) {
                reorderedList.add(info);
            }
        }
    ...
    private static ChannelDisplayInfo getChannelByPrefix(String prefixToMatch, Map<String, ChannelDisplayInfo> channelMap) {
        for (String key : channelMap.keySet()) {
            if (key.startsWith(prefixToMatch)) {
                ChannelDisplayInfo value = channelMap.get(key);
                return value;
            }
        }
        return null;
    ...
    private static List<ChannelDisplayInfo> getChannels(ImageDisplay display, boolean allChannels) {
        ...
        if (imageData.getServer().isRGB()) {
            if (imageData.getColorDeconvolutionStains() != null) {
                return display.availableChannels()
                        .stream()
                        .filter(ReorderableMiniViewer::isColorDeconvolutionChannel)
                        .collect(Collectors.toList());
            } else {
                return display.availableChannels()
                        .stream()
                        .filter(ReorderableMiniViewer::isRGBChannel)
                        .collect(Collectors.toList());
            }
        } else {
     ...
     * @return
     */
    static ContextMenu createPopup(final ReorderableMiniViewer.MiniViewerManager manager) {
    ...
    /**
     * Create a {@link ReorderableMiniViewer.MiniViewerManager} associated with a specified viewer.
     * @param viewer
     * @return
     * @since v0.4.0
     */
    public static ReorderableMiniViewer.MiniViewerManager createManager(QuPathViewer viewer) {
        return new ReorderableMiniViewer.MiniViewerManager(viewer, Collections.emptyList());
    }

    /**
     * Create a {@link ReorderableMiniViewer.MiniViewerManager} displaying multiple channels and
     ...
     * @since v0.4.0
     */
    public static ReorderableMiniViewer.MiniViewerManager createManager(QuPathViewer viewer, Collection<? extends ChannelDisplayInfo> channels) {
        return new ReorderableMiniViewer.MiniViewerManager(viewer, channels);
    }
    ...
    public static class MiniViewerManager implements EventHandler<MouseEvent> {
         ...
         *
         * @deprecated use {@link ReorderableMiniViewer#createManager(QuPathViewer, Collection)} instead.
         */
        @Deprecated
        ...
        Pane createPane(ReorderableMiniViewer.MiniViewerManager.MiniViewer canvas, int col, int row, int colSpan, int rowSpan) {
        ...
        void requestFullUpdate() {
            for (ReorderableMiniViewer.MiniViewerManager.MiniViewer viewer : miniViewers) {
        ...
        class MiniViewer extends Canvas {
            ...
            public MiniViewer(ImageRenderer renderer) {
                ...
                nameBinding.bind(Bindings.createStringBinding(() -> {
                            if (renderer instanceof ReorderableMiniViewer.MiniViewerManager.ImageDisplaySingleChannelRenderer) {
                                ReorderableMiniViewer.MiniViewerManager.ImageDisplaySingleChannelRenderer channelRenderer = (ReorderableMiniViewer.MiniViewerManager.ImageDisplaySingleChannelRenderer)renderer;
